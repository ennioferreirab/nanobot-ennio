...
class TestPlanningWorkerLeadAgentModel:
    ...
    def _run_worker(self, bridge: MagicMock, plan: object, *, task: dict | None = None) -> list:
        """Helper: run process_task and return captured planner.plan_task calls."""
        if task is None:
            task = _make_task()
        materializer = _make_materializer()
        dispatcher = _make_dispatcher()
        ...
        async def _run():
            ...
            with (
                ...
                patch("mc.runtime.workers.planning.TaskPlanner") as planner_cls,
                patch("mc.runtime.workers.planning.TierResolver") as tier_resolver_cls,
            ):
                mock_resolver = tier_resolver_cls.return_value
                # By default: raise for tier references so test can configure
                mock_resolver.resolve_model.side_effect = lambda m: (
                    None
                    if m is None or not (m or "").startswith("tier:")
                    else m.replace("tier:", "resolved-")
                )
                mock_resolver.resolve_reasoning_level.return_value = None
                planner = planner_cls.return_value
                planner.plan_task = AsyncMock(return_value=plan)
                await worker.process_task(task)
                if planner.plan_task.called:
                    captured_calls.append(planner.plan_task.call_args)
